...
APIDICT_PATH = "../../../mnt/storage/yanjie/drebin_mal_api"
MOV_PATH = "../../../mnt/storage/yanjie/DREBIN_Malfeature"
PICKED_PATH = "../../../mnt/storage/yanjie/drebin_api/malfeature_picked"
...
class Analysis:
    ...
    def process_one(self, args):
        apk = args
        apk_sha256 = os.path.split(apk)[-1][:-8]
        try:

            f = open(apk, "rb")
            apidict_md5 = hashlib.sha256(f.read()).hexdigest()
            f.close()

            self.lock.acquire()
            if not apidict_md5 in self.api_md5:
                # cp
